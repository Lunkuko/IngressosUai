...
class CanvasLibrarie {
	...
	*/
	...
	{ 
		switch ( $this->formato )
		{
			...
			case 2:
				...
				////switch ($ort) {
				////case 3:
					////$image = imagerotate( $this->img, 180, 0);
					////break;
				////case 6:
					////$image = imagerotate( $this->img, -90, 0);
					////break;
				////case 8:
					////$image = imagerotate( $this->img, 90, 0);
					////break;
				////}
				//$image = imagerotate( $this->img, -90, 0);
				//imagejpeg($image, $this->origem, 90);
				//$this->img = $image;
				//print_r( $exif );
			...
			case 3:
				...
				// correcao fundo preto 
				//$preto = imagecolorallocate($this->origem , 0, 0, 0);
				//$this->img = @imagecolortransparent ($this->origem , $preto);
	...
	public function maketransparency(){
	...
	*/
	private function redimensionaSimples()
	{
		...
		// by bruno santos
		if($this->extensao =="png" or $this->extensao =="gif"){
	...
	*/
	...
	{
		$width = $this->nova_largura;
		$height = $this->nova_altura;
		...
		$this->img_temp = imagecreatetruecolor($width, $height);
		imagecopyresampled($this->img_temp, $this->img, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
		
		$this->img = $this->img_temp;
	}


	private function redimensionaPreenchimento_old()
	{
		// cria imagem de destino temporária
		$this->img_temp	= imagecreatetruecolor( $this->nova_largura, $this->nova_altura );
		...
		// --------------------------------------------------
		$newWidth	= $this->nova_largura;
		$newHeight	= $this->nova_altura;
		...
		//imagecopyresampled( $this->img_temp, $this->img, $dif_x, $dif_y, 0, 0, $optimalWidth, $optimalHeight, $this->largura, $this->altura );
		imagecopyresampled( $this->img_temp, $this->img, 0, 0, 0, 0, $optimalWidth, $optimalHeight, $width, $height );
		$this->img	= $this->img_temp;
	...
	*/
	...
	/**
	 * marca_imagem (developer: marcio lima : guardiao)
	 * @args 
	 *	@photo		- imagem original
	 *	@logotipo	- imagem da marca dagua 
	*/
	public function marca_imagem($args){
		$image_base = (isset($args["photo"]) ? $args["photo"] : "");
		$logotipo = (isset($args["logotipo"]) ? $args["logotipo"] : "");
		$extension = pathinfo($image_base,PATHINFO_EXTENSION);
		
		$watermark_image = imagecreatefrompng( $logotipo );
		if($extension == 'jpg' || $extension == 'jpeg') { $image = imagecreatefromjpeg($image_base); }
		if($extension == 'png') { $image = imagecreatefrompng($image_base); }

		$margin_right = 10; 
		$margin_bottom = 10;

		list( $marca_w, $marca_h ) = getimagesize( $logotipo );
		...
		$dwidth  = imagesx($image);
		$dheight = imagesy($image);

		//pega o tamanho da imagem que vai ser centralizada
		$mwidth  = imagesx($watermark_image);
		$mheight = imagesy($watermark_image);
		...
		imagecopy($image, $watermark_image, $xPos, $yPos, 0, 0, $watermark_image_width, $watermark_image_height); 
		
		if($extension == 'jpg' || $extension == 'jpeg') { imagejpeg($image, $image_base, 85); }
		if($extension == 'png') { imagepng($image, $image_base, 8); }
	...
	*/
	public function grava( $destino='', $qualidade = 100 )
	{
		...
		if ( !isset( $extensao_destino ) )
		{
		 ...
		 // correcao bruno 20/10/16> alteracao da variavel $extensao_destino para $this->formato (problema fundo preto png)
